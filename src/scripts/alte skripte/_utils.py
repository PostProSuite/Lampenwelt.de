...
def load_config():
    ...
    if not os.path.exists(env_file):
        raise FileNotFoundError(
            ...
            f"Bitte führen Sie aus: cp 00-scripts/config/config.env.example 00-scripts/config/config.env\n"
            f"Dann tragen Sie Ihre Zugangsdaten in die Datei ein.\n"
            f"Pfad: {env_file}"
        )
...
def get_paths():
    ...
    return {
        'base':             base,
        'scripts':          os.path.join(base, '00-scripts'),
        'input_batchfiles': os.path.join(base, '01-Input-Batchfiles'),
        'excel_input':      os.path.join(base, '02-excel-input'),
        'web_check':        os.path.join(base, '05-web-check'),
        'final_images':     os.path.join(base, '08-FINAL-Images'),
        'upload':           os.path.join(base, '10-Upload'),
        'json':             os.path.join(base, '00-scripts', 'JSON'),
